...
func verifyPasswords(passwords: String) -> Int {
    let formattedPasswords = passwords.components(separatedBy: "\n")
    
    var validPasswordCount: Int = 0
    
    for password in formattedPasswords {
        let line = parseLine(input: password)
        
        if checkPassword(firstPosition: line.firstPosition, secondPosition: line.secondPosition, character: line.character, password: line.password) {
...
func parseLine(input: String) -> (firstPosition: Int, secondPosition: Int, character: Character, password: String) {
    let parsedLine = input.components(separatedBy: CharacterSet(["-", " ", ":"]))
    
    let firstPosition = Int(parsedLine[0])! - 1
    let secondPosition = Int(parsedLine[1])! - 1
    let char = Character(parsedLine[2])
    let password = parsedLine[4]
    
    return (firstPosition, secondPosition, char, password)
}

func checkPassword(firstPosition: Int, secondPosition: Int, character: Character, password: String) -> Bool {
    let firstPositionStringIndex = password.index(password.startIndex, offsetBy: firstPosition)
    let secondPositionStringIndex = password.index(password.startIndex, offsetBy: secondPosition)
    
    return (password[firstPositionStringIndex] == character) != (password[secondPositionStringIndex] == character)
}

print(verifyPasswords(passwords: passwords))
