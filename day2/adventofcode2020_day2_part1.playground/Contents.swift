...
func verifyPasswords(passwords: String) -> Int {
    ...
    for password in formattedPasswords {
        ...
        if checkPassword(min: line.min, max: line.max, character: line.character, password: line.password) {
...
func parseLine(input: String) -> (min: Int, max: Int, character: Character, password: String) {
    let parsed = input.components(separatedBy: CharacterSet(["-", " ", ":"]))
    
    let min = Int(parsed[0])!
    let max = Int(parsed[1])!
    let char = Character(parsed[2])
    let password = parsed[4]
    
    return (min, max, char, password)
}

func checkPassword(min: Int, max: Int, character: Character, password: String) -> Bool {
