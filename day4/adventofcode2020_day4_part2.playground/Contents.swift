...
enum PassportField: String, CaseIterable {
    ...
    func doesExistIn(_ line: String) -> Bool {
        switch self {
        case .cid:
            return true
        default:
            return line.contains(rawValue)
        }
    }
    ...
    func isValidValue(_ value: String) -> Bool {
        ...
        case .hgt:
            if value.contains("cm") {
                let num = Int(value.components(separatedBy: "cm")[0])!
                return (num >= 150) && (num <= 193)
            } else if value.contains("in") {
                let num = Int(value.components(separatedBy: "in")[0])!
        ...
        case .hcl:
            if value.contains("#") {
                let hex = value.components(separatedBy: "#")[1]
                return hex.isAlphanumeric && hex.count == 6
            } else {
                return false
            }
        case .ecl:
            ...
            return
                ...
                value.contains("oth")
        case .pid:
            return value.isAlphanumeric && value.count == 9
        case .cid:
            return true
        }
    }
...
for line in lines {
    let fieldsLine = line.components(separatedBy: CharacterSet.whitespacesAndNewlines)
    
    var isValid = fieldsLine.count >= 7 && PassportField.allCases.allSatisfy({ $0.doesExistIn(line) })

    for line in fieldsLine {
        guard isValid else { break }

        let parsedLine = line.components(separatedBy: ":")
        
        guard let field = PassportField(rawValue: parsedLine[0]) else {
            isValid = false
            break
        }

        isValid = field.isValidValue(parsedLine[1])
    }
    
    if isValid { numberValid += 1 }
}
